...
describe('Flowchart Factory', function () {
    ...
    it('creates a flowchart from an array', function (array|bool $end, array|string $goto, array|string $error) use ($factory) {
        ...
        expect([...$flowchart->context])->toBe(['foo' => 'bar'])
            ...
            ->and($flowchart->entrypoint->whenResultIs(false))->toBeInstanceOf(EndFlow::class)
            ->and($flowchart->entrypoint->whenResultIs(false)->result)->toBeFalse()
            ->and($flowchart->entrypoint->whenResultIs(true))->toBeInstanceOf(DecisionNode::class)
            ->and($flowchart->entrypoint->whenResultIs(true)->checkerServiceId)->toBe('some.other.checker.service')
            ->and($flowchart->entrypoint->whenResultIs(true)->whenResultIs(true))->toBeInstanceOf(GotoNode::class)
            ->and($flowchart->entrypoint->whenResultIs(true)->whenResultIs(true)->id)->toBe('root')
            ->and($flowchart->entrypoint->whenResultIs(true)->whenResultIs(false))->toBeInstanceOf(DecisionNode::class)
            ->and($flowchart->entrypoint->whenResultIs(true)->whenResultIs(false)->checkerServiceId)->toBe('yet.another.checker.service')
            ->and($flowchart->entrypoint->whenResultIs(true)->whenResultIs(false)->whenResultIs(false))->toBeInstanceOf(RaiseError::class)
            ->and($flowchart->entrypoint->whenResultIs(true)->whenResultIs(false)->whenResultIs(false)->message)->toBe('Ooops')
            ->and($flowchart->entrypoint->whenResultIs(true)->whenResultIs(false)->whenResultIs(true))->toBeInstanceOf(UnhandledStep::class);
    })->with(function () {
        $dataset = [
            'end' => [
                false,
...
describe('Flowchart Factory -> Options', function () {
    ...
    it('accepts a default checker service', function () use ($factory) {
        ...
        expect($flowchart->entrypoint->checkerServiceId)->toBe('default.checker.service')
            ->and($flowchart->entrypoint->whenResultIs(true)->checkerServiceId)->toBe('some.checker.service')
            ->and($flowchart->entrypoint->whenResultIs(false)->checkerServiceId)->toBe('default.checker.service');
    });
});
...
describe('Flowchart Factory Validation', function () {
    ...
    it('accepts a boolean shortcut for `when@true` or `when@false`', function () use ($factory) {
        ...
        expect($flowchart->entrypoint->whenResultIs(true))->toBeInstanceOf(EndFlow::class)
            ->and($flowchart->entrypoint->whenResultIs(true))->toEqual(new EndFlow(true))
            ->and($flowchart->entrypoint->whenResultIs(false))->toBeInstanceOf(EndFlow::class)
            ->and($flowchart->entrypoint->whenResultIs(false))->toEqual(new EndFlow(false))
        ;
    });
